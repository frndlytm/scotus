...
from scotus import constants, data, trainer, plotting
from scotus.model import DialogueRNN
...
def train(
    ...
    output_path = os.path.join(constants.MODELS_DIR, run_id)
    ...
    model = DialogueRNN(...).to(constants.DEVICE)
    ...
    if plot_result:
        plotting.plot_result(
            os.path.join(output_path, f"{run_id}.log.jsonl"),
            units, dropout, lr, os.path.join(output_path, f"{run_id}")
        )
